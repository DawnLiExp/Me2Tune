...
struct PlaybackStateManagerTests {
    ...
    @Test("移动到下一首 - 成功")
    func testMoveToNextIndexSuccess() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("移动到下一首 - 已在最后")
    func testMoveToNextIndexAtEnd() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("移动到下一首 - 未设置索引")
    func testMoveToNextIndexNoIndex() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("移动到上一首 - 成功")
    func testMoveToPreviousIndexSuccess() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("移动到上一首 - 已在开头")
    func testMoveToPreviousIndexAtStart() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("移动到上一首 - 未设置索引")
    func testMoveToPreviousIndexNoIndex() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
    ...
    @Test("calculateNextIndex - 单曲循环")
    func testCalculateNextIndexRepeatOne() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        
        manager.switchToPlaylist()
        
        let nextIndex = manager.calculateNextIndex(at: 1, repeatMode: .one)
        
        #expect(nextIndex == 1)
    }
    
    @Test("calculateNextIndex - 列表循环（中间）")
    func testCalculateNextIndexRepeatAllMiddle() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        
        manager.switchToPlaylist()
        
        let nextIndex = manager.calculateNextIndex(at: 1, repeatMode: .all)
        
        #expect(nextIndex == 2)
    }
    
    @Test("calculateNextIndex - 列表循环（结尾）")
    func testCalculateNextIndexRepeatAllEnd() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        
        manager.switchToPlaylist()
        
        let nextIndex = manager.calculateNextIndex(at: 2, repeatMode: .all)
        
        #expect(nextIndex == 0)
    }
    
    @Test("calculateNextIndex - 不循环（中间）")
    func testCalculateNextIndexOffMiddle() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        
        manager.switchToPlaylist()
        
        let nextIndex = manager.calculateNextIndex(at: 1, repeatMode: .off)
        
        #expect(nextIndex == 2)
    }
    
    @Test("calculateNextIndex - 不循环（结尾）")
    func testCalculateNextIndexOffEnd() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        
        manager.switchToPlaylist()
        
        let nextIndex = manager.calculateNextIndex(at: 2, repeatMode: .off)
        
        #expect(nextIndex == nil)
    }
    
    @Test("calculateNextIndex - 使用当前索引")
    func testCalculateNextIndexWithCurrentIndex() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
        ...
        let nextIndex = manager.calculateNextIndex(repeatMode: .all)
        
        #expect(nextIndex == 2)
    }
    
    @Test("calculateNextIndex - 当前索引为 nil")
    func testCalculateNextIndexNoCurrentIndex() throws {
        let (manager, playlistManager, _, _) = try setupWithPlaylistTracks(count: 3)
        _ = playlistManager
